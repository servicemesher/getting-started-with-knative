...
Knative的 Serving(服务)组件是如何从容器到 URL 的，而 Build(构建)组件是如何从源代码到容器的。Build resource 允许您定义如何编译代码和构建容器，而不是指向预构建的容器镜像。这确保了在将代码发送到您选择的容器镜像库之前以一致的方式编译和打包代码。有一些新的组件，我们将在本章介绍：
...
> [备注] 在编写本文时，有一些活动的工作要迁移[Build Pipelines（构建流水线）](http://bit.ly/2SqnuTc)，对构建中的流水线进行重构更类似于 CI/CD 流水线的 Knative。这意味着除了编译和打包代码外，Knative中的构建还可以轻松地运行测试并发布这些结果。请密切关注Knative的未来版本，了解这一变化。
...
首先要注意的是，`username` 和 `password` 在传递给 Kubernetes 时都是 base64 编码的。我们还注意到，我们使用`basic-auth`根据 Docker Hub 进行身份验证，这意味着我们将使用用户名和密码进行身份验证，而不是类似于 access token（访问令牌）的东西。此外，Knative 还附带了开箱即用的`ssh-auth`，这允许我们使用SSH私钥如果我们想从私有Git存储库中拉取代码。
...
让我们从 Hello World 应用程序开始。这是一个简单的 Go 应用程序，它监听端口8080并以“Hello from Knative!”作为HTTP GET 请求的回应。它的全部代码如[Example 3-4]()所示。
...
我们还将编写一个 Dockerfile 来构建我们的代码和容器，如[Example 3-5]()所示。
...
在前面的[第2章]()中，我们在本地构建了容器并手动将其推送到容器镜像库。然而，Knative 为我们在Kubernetes 集群中使用 Builds（构建）来完成这些步骤提供了一种更好的方式。与 Configurations （配置）和 Routes（路由）一样，Builds（构建）也可以简单地作为 Kubernetes 自定义资源(CRD)来通过YAML定义的方式实现。在深入研究每个组件之前，让我们看一看[Example 3-6]()，看看 Build（构建）的配置是什么样的。
...
在构建步骤之前，我们还会看到定义源代码位置的源代码部分。如今，Knative发布了三个代码源选项：

* git
Git 仓库，可以选择使用参数来定义分支、标记或提交SHA。
...
我们只需要安装一个额外的组件，即 Build Template（构建模板）。我们将在“Build templates”一节中更深入地介绍这些内容，但是现在，我们将继续安装我们在YAML中定义的那个，在本例中是 Kaniko Build Template（构建模板）[见 Example 3-7]()。
...
通过应用模板，我们可以像在Serving（服务）示例中那样部署服务配置[见 Example 3-8]()。
...
在 [Example 3-6]() 中，我们使用了一个 Build Template（构建模板），但从未真正解释过 Build Template（构建模板）是什么或它做什么。简单来说,Build Template（构建模板）是可共享的、封装的、参数化的构建步骤集合。目前，Knative已经支持多个 Build Template（构建模板），包括：

* Kaniko

> 在运行的容器中构建容器镜像，而不依赖于运行Docker daemon（守护进程）。
...
虽然这并不是可用模板的完整列表，但是我们可以轻松地集成 Knative 社区开发的新模板。安装Build Template（构建模板）和应用YAML文件安装 Service（服务）、Route（路由）或 Build configuration（构建配置）一样简单：
...
然后我们可以像其他configura配置一样应用[Example 3-6]()来部署我们的应用程序，同时向它发送请求，就像我们在[第2章]()中所做的那样：
...
让我们进一步观察 Build Template（构建模板），在 [Example 3-9]() 中继续使用 Kaniko 作为参考。
...
Build Template（构建模板）的 `steps` 部分具有与 Build（构建）完全相同的语法，只是使用命名变量进行模板化。实际上，我们将看到除了用变量替换路径之外，`steps` 部分看起来非常类似于 [Example 3-6]() 的模板部分。`parameters` 部分在Build Template（构建模板）所期望的参数周围放置了一些结构。Kaniko Build Template（构建模板）需要一个定义在何处推送容器镜像的 `IMAGE` 参数，但是有一个可选的 `DOCKERFILE` 参数，如果没有定义该参数，则提供一个默认值。
...
我们已经花了很多时间来构建和运行我们的应用程序，但是 serverless 的最大承诺之一是，它可以使您的服务轻松地连接到事件源。在下一章中，我们将研究 Knative 的Eventing（事件）组件以及开箱即用的所有可有事件源。
